...
public class AbstractSmartCallerReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        this.intent = intent;
        //Log.d("rohit", "Action "+intent.getExtras().getString("android.intent.extra.PHONE_NUMBER"));
        //We listen to two intents.  The new outgoing call only tells us of an outgoing call.  We use it to get the number.
        if (intent.getAction().equals("android.intent.action.NEW_OUTGOING_CALL")) {
            savedNumber = intent.getExtras().getString("android.intent.extra.PHONE_NUMBER");
            //Log.d("rohit", "saved number "+savedNumber);
        } else {
        ...
        switch (state) {
            ...
            case TelephonyManager.CALL_STATE_OFFHOOK:
                ...
                if (lastState != TelephonyManager.CALL_STATE_RINGING) {
                    isIncoming = false;
                    callStartTime = new Date();
                    //Log.d("rohit", "outgoing call "+savedNumber);
